...
public class Game extends JPanel implements Runnable {
	...
	static ArrayList<Shoot> shoots = new ArrayList<>();
	Shoot playerJet;
	static ArrayList<Shoot> bullets = new ArrayList<>();
	static List<Shoot> bulletsList = Collections.synchronizedList(bullets);
	ArrayList<Shoot> lives = new ArrayList<>();
	List<Shoot> livesList = Collections.synchronizedList(lives);
	...
	public Game() {
		...
		shoots.add(new Shoot("Mountain.jpg", 0, 0, 616, 616));
		...
		playerJet = new Shoot("helicopter-animation.gif", 250, 490, 80, 80);
		shoots.add(playerJet);

		lives.add(new Shoot("heart.png", 450, 500, 40, 40));
		lives.add(new Shoot("heart.png", 490, 500, 40, 40));
		lives.add(new Shoot("heart.png", 530, 500, 40, 40));
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < shoots.size(); i++) {
			Shoot s = shoots.get(i); // draws the enemies and fighter jet
				...
				synchronized (this) {
					for (int i = bulletsList.size() - 1; i >= 0; i--) {
						...
						if (b.isPlayers()) { // checks whether the bullet belongs to the enemy or not
							for (Shoot a : shoots) {
								if (a instanceof EnemyJet) { // checks the object in sprite array is an object of
																	// enemy
									EnemyJet e = (EnemyJet) a;
									if (b.checkCollision(Bullet.getBounds(e))) { // checks if the bullet from player and
																				// the enemy jet collapse
										shoots.remove(e); // if they collapse remove the enemy jet e
							...
							if (b.checkCollision(Bullet.getBounds(playerJet))) { // checks whether the bullet collapse
																				// with the enemy jet
